...
def train(args):
    ...
    # For plotting training vs validation EPE3D every 1/10th of total steps
    eval_interval = max(args.num_steps // 10, 1)
    eval_steps = []
    train_epe3d_eval = []
    val_epe3d_eval = []
    train_epe3d_history = []
    while total_steps < args.num_steps:
        for i_batch, train_data_blob in enumerate(train_loader):
            ...
            train_epe3d_history.append(metrics['epe3d'])
            ...
            # run validation every eval_interval steps (≈ every 1/10th of training)
            if (total_steps % eval_interval == 0 or total_steps == args.num_steps) and total_steps > 0:
                # average training EPE3D over the last eval_interval steps
                window = train_epe3d_history[-eval_interval:] if len(train_epe3d_history) >= eval_interval else train_epe3d_history
                avg_train_epe3d = float(np.mean(window)) if len(window) > 0 else 0.0

                print(f"\nRunning validation at step {total_steps} (avg train EPE3D over last {len(window)} steps: {avg_train_epe3d:.4f})")
                model.eval()
                val_results = test_sceneflow(model.module)
                model.train()

                val_epe3d = float(val_results.get('epe3d', 0.0))
                eval_steps.append(total_steps)
                train_epe3d_eval.append(avg_train_epe3d)
                val_epe3d_eval.append(val_epe3d)
                print(f"Validation EPE3D at step {total_steps}: {val_epe3d:.4f}")
    ...
    # Plot training vs validation EPE3D over evaluation steps (every ~1/10th of num_steps)
    if len(eval_steps) > 0:
        plt.figure(figsize=(8, 5))
        plt.plot(eval_steps, train_epe3d_eval, marker='o', label='Train EPE3D')
        plt.plot(eval_steps, val_epe3d_eval, marker='s', label='Val EPE3D')
        plt.xlabel('Training step')
        plt.ylabel('EPE3D')
        plt.title('Train vs Val EPE3D (every ~1/10th of steps)')
        plt.grid(True)
        plt.legend()
        plt.tight_layout()
        plot_path = os.path.join('checkpoints', f"{args.name}_epe3d_train_val.png")
        plt.savefig(plot_path, dpi=150)
        print(f"Saved EPE3D train/val plot to: {plot_path}")

    print("Training complete. Running final evaluation...")
    model.eval()
    _ = test_sceneflow(model.module)
